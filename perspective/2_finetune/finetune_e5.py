# finetunes those embedders using AutoModel
import logging
import os
import torch
import random
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for k in ['query', 'pos', 'neg']:
            cur_inputs = {
                ...
                'token_type_ids': inputs[f'{k}_token_type_ids'],
            }
            cur_outputs: BaseModelOutput = model(**cur_inputs)
            # get averaged embedding with attention_mask
            cur_results[k] = pool(cur_outputs.last_hidden_state, cur_inputs['attention_mask'], 'avg')
